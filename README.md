...
- 瀑布流
    - 两列、图片高度不一致、有落差感
        滚动加载更多，实现图片懒加载
        接口： /api/images?page=${n}
        使用Mock随机生成数据，通过es6 中 Array.from 方法创建数组
            Array.from中的两个参数 ，
                第一个参数：类数组对象（Array-like object），具有 length 属性
                第二个参数：映射函数，用于将类数组对象的每个元素转换为指定格式
        使用 page-pageIndex 作为唯一id 索引
    - 如何分配两列
    - 加载更多
        通过在底部添加一个加载更多的元素，当用户滚动到这个元素时，触发加载更多的操作
        intersection observer  监听元素是否进入视口 
        当组件卸载时，使用observer.disconnect() 取消所有的观察，防止内存泄漏
    - 下拉刷新
    - css
        object-fit: cover ：该属性用于控制图片在容器中的缩放和裁剪行为
            - 保持图片原始宽高比
            - 将图片缩放到完全覆盖容器（可能会裁剪图片边缘）
            - 确保容器被完全填充，不会出现空白区域
        background-color: #eee;
            作用是为图片添加一个背景颜色，当图片加载完成之前，背景颜色会显示出来，避免图片加载完成之前，容器为空，导致页面布局发生变化。
    - 


- toast 组件 提示消息
    虽然react-vant中有这个功能组件，但是我需要自定义的样式，所以需要自己封装一个toast组件
    - UI 
    - 使用js来显示出来，实现跨层级通信
        订阅者发布者
        pnpm i mitt
...
2. 前端智能
    ...
    - 文生图，prompt设计
        帮助冰球社区玩家创作图片
        - 使用coze工作流，实现智能编排ai工作流程
            - 创建工作工作流 ： 上传图片，根据宠物生成宠物棒球运动员照片
        - 代码节点
            校验参数和逻辑，返回运动的结果
        - 图片生成流程
            - 图片理解插件 计算机视觉
            - 特征提取
        - 使用工作流
            api  https://api.coze.cn/v1/workflow/run
            workflow_id=7533135222833938482
            1. 使用 formDate 上传图片首先需要上传到coze 的服务器中，并获取coze云端存储图片地址的id，而不能直接给工作流。
            2. 然后 通过 workflow_id + workflowURL + token 来调用工作流。实现图片生成
...
5. 用户体验
    ...
    - 组件粒度划分
    - 骨架屏
    - 文件上传的图片预览 使用html5 FileReader
### 遇到的问题并如何解决
- chat message中遇到message覆盖问题 —— 这是一个闭包陷阱的问题
- 如何升级瀑布流
    - 因为两列的高度是按奇偶来分配的，若有一列的高度过于长，在底部会导致一边有图片一边没有图片。
    使用两个数组，分别存储两列的图片，当有新图片加载时，将图片分配到高度较短的那一列。
    - intersectionObserver 使用了两次，重复使用。存在问题 使用hook 封装
